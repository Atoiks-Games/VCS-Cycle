...
public abstract class VerticalPage extends Page {
    ...
    public void render(IGraphics g) {
        ...
        g.clearGraphics();
        if (background != null) g.drawImage(background, textOnRight ? 0 : App.WIDTH / 2, 0);
        ...
        if (charProgress > 0 || lineProgress > 0) {
            ...
            for (int i = 0; i < bound; ++i) {
                ...
                if (i < lineProgress) {
                    ...
                    g.drawString(s, getMessageRenderX(), baseHeight + i * FONT_SIZE);
                    ...
                    g.drawString(actualMessage, getMessageRenderX(), baseHeight + i * FONT_SIZE);
            ...
            if (doneScrolling()) {
                ...
                for (int i = 0; i < optDispCount; ++i) {
                    ...
                    g.drawString(s, getMessageRenderX() + 20, h);
                    optHeight[offset] = h - FONT_SIZE / 2 + 2;
                }

                if (renderSelector && option >= 0 && option < optHeight.length) {
                    g.setColor(optionsColor);
                    g.fillCircle(getMessageRenderX() + 10, optHeight[option], 5);
                }
                ...
                if (optSectCount > 0) {
                    ...
                    g.drawString("Option Page (" + (optSect + 1) + "/" + (optSectCount + 1) + ")",
                            getMessageRenderX() - 10, App.HEIGHT - 4);
                }
            }
        }
    }
